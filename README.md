...
5. **Compatibility with Claude Code GitHub Actions**

   EDAMAME Posture works with [Claude Code GitHub Actions](https://code.claude.com/docs/en/github-actions) just as it does with any other runner-based CI/CD step. Place the EDAMAME Posture setup step **before** the Claude Code action so the runner is hardened, monitored, and granted access to code only after posture initialization is complete. Claude Code then operates on a secured runner with full network visibility.
   ...
   steps:
     - name: Setup EDAMAME Posture
       uses: edamametechnologies/edamame_posture_action@v1
       with:
         edamame_user: ${{ vars.EDAMAME_POSTURE_USER_VM }}
         edamame_domain: ${{ vars.EDAMAME_POSTURE_DOMAIN_VM }}
         edamame_pin: ${{ secrets.EDAMAME_POSTURE_PIN_VM }}
         edamame_id: ${{ github.run_id }}
         checkout: true
   ...
   This ordering ensures that EDAMAME Posture hardens the runner and begins network capture before Claude Code accesses the repository or makes any network calls. All traffic generated by the Claude Code action is monitored and subject to the same whitelist, blacklist, and anomaly detection policies as any other workflow step.
